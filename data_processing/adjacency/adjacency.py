...
# adjusted to parse Canadian Province data
...
for index, country in gdf.iterrows():   
    ...
    # print progress
    print("Processing Region - " + REGION_ID_STR + " = " + str(gdf.at[index, REGION_NAMESTR_LABEL]))
...
verbose_df = gpd.GeoDataFrame(gdf[[REGION_ID_STR, REGION_NAMESTR_LABEL, "neighbors"]])
...
for index, region in neighborhood_df.iterrows():
    ...
    # check if the neighbor_str is empty (no neighbors)
    if not neighbor_str:
        print("Warning: PRUID " + current_region + " has no neighbors")
        continue
